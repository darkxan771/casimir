...
from .labels import Label, TooLongError, complete_list
...
def cht_SUSO(n: int, L: Sequence[int]) -> Label:
    if len(L) >= n:
        raise TooLongError
    M = complete_list(n, L)
    if n % 2 == 1:
        return Partition([M[i] - M[n - 1 - i] for i in range(int((n - 1) / 2))])
    else:
        return Signature([M[i] - M[n - 1 - i] for i in range(int(n / 2))])


def cht_SU2SP(n: int, L: Sequence[int]) -> Partition:
    if len(L) >= 2 * n:
        raise TooLongError
...
def cht_GrC(p: int, q: int, L: Sequence[int]) -> tuple[Signature, Partition]:
    M = complete_list(p + q - 1, L)
    return (Signature(M[:p]), Partition(M[p:]))


def cht_GrH(p: int, q: int, L: Sequence[int]) -> tuple[Partition, Partition]:
    M = complete_list(p + q, L)
    return (Partition(M[:p]), Partition(M[p:]))


compute_highest_type: dict[str, Callable] = {}
compute_highest_type["SU/SO"] = cht_SUSO
compute_highest_type["SU2/SP"] = cht_SU2SP
compute_highest_type["SP/U"] = lambda n, L: Signature(complete_list(n, L))
compute_highest_type["GrR"] = cht_GrR
compute_highest_type["GrC"] = cht_GrC
compute_highest_type["GrH"] = cht_GrH
...
class Partition(Label):
    ...
    def hypocasimir(self, X: SymmetricSpace):
        ...
        if X.stype == "structure":
            ...
            if X.type == "SP/U":
                coeff *= 2
            CK = self.highest_K_type(X).casimir(K, coeff)
        else:
            L, M = self.highest_K_type(X)
            if X.type == "GrC":
                n, p, q = X.n, X.p, X.q
                K1, K2 = LieGroup("SU", p), LieGroup("SU", q)
                L1 = Partition([a - L.terms[-1] for a in L.terms[:-1]])
                CK = L1.casimir(K1, coeff) + M.casimir(K2, coeff)
                CK -= Rational(
                    (q * L.size - p * M.size) ** 2, 2 * n * n * p * q
                )
            else:
